# 5. Combined search suggestions across entities, without server-side LLM ranking
...
The catalogue this runs against is small and static: it's loaded once, connected, and cached in memory for 24 hours ([1](1-backend-for-frontend-with-cached-dataset.md)), so search executes over a few hundred in-process records, not a datastore that needs external retrieval.
