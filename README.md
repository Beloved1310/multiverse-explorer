...
For horizontally scaled deployments, set `REDIS_URL` to enable a shared 24-hour catalogue cache. Redis is optional: if it is unavailable, the app continues with the existing Next.js cache and snapshot fallback. The rationale is in [docs/decisions/8](docs/decisions/8-redis-shared-read-cache.md).
...
_Somewhere in the multiverse there's a Rick who wrote this whole app with zero bugs on the first try. This is not that universe._ 🌀
